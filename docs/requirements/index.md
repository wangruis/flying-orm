...
- 当前进展：动态表单 CRUD、分页、排序、count、逻辑删除、乐观锁和批量写入已经闭环；DDL 支持建表、安全 `createOrAlter`、字段及索引差异、显式重命名和危险变更审核，并能返回正向计划、回滚计划、风险、批准指纹和逐步执行结果。五种数据库的当前 V1 DDL 契约已经完成真实库认证；更高级在线迁移仍属于后续版本。
...
- 状态：V1 历史桥接方案已结束；V2 原生 JDBC 同步内核已接入
...
- 当前进展：`RowMapper`、`EntityValues`、响应式/同步 Repository、flying-orm 自有实体注解、生命周期契约和字段感知 codec 已经闭环。JSON、PostgreSQL Array、BLOB/CLOB 和 Vector 的当前契约均已完成真实库验证；内部 `MappingPlan` 保持包级可见，不进入 V1 公共兼容承诺。
...
- 当前进展：普通 SQL 默认启用 SQL 超时、最大结果行数、累计结果内存、单个 LOB 大小和连接获取等待上限；动态表单解码后的 BLOB/CLOB 也会重新检查总内存，避免物化后绕过限制。DDL 使用独立的更宽但有限上限。批量写入同时限制总行数、分片、并发、内存与结果分片数；Repository 需要保留生命周期实体时，实体和映射后的参数对象共用同一内存预算。确需解除保护时必须显式选择 `unlimited()`。
...
- 状态：已完成
- 目标：在不打断 V1 发版收口的前提下，补 PostgreSQL Vector、受控 SQL 模板逃生口、更完整的 flying-orm 自有注解映射语义、schema/独立库/RLS 隔离，以及迁移回滚和在线 DDL 审核。
- 当前进展：五项代码能力和最终认证均已完成。SQL 模板只能由服务端注册并参数绑定；迁移审核会生成回滚计划、风险和精确批准指纹；pgvector、Schema/RLS 会话设置及连接池清理已经在真实 PostgreSQL 环境通过。详细入口见 `docs/v1-post-release-enhancements.md`。

## R-024 发布后企业增强

- 目标：补复合游标分页、数据级迁移补偿、正式 Oracle/SQL Server 支持、缓存指标桥和驱动特殊值/列别名适配，不回退 V1 状态。
...
- 状态：已实现并完成 V1.0.0 验收
- 来源：用户要求把原规划在 v1.0.2 的能力全部前移到 v1.0.0。
...
- 详细记录：`docs/requirements/2026-08-03-v1.0.0-hardening-lambda-spring-schema.md`。

## R-027 全局类规模约束与 v1.0.1 大型类治理

- 状态：全局规则与 flying-orm V1.0.1 治理均已完成
...
- 当前进展：全局规则已写入 `AGENTS.md`。V1.0.1 已完成 15 个大型类型的职责治理，没有把职责搬进新的生产大类；
  V1.0.0 公开签名继续兼容，并新增 V1.0.1 精确 API 快照。功能、SQL、参数顺序、Scope、事务、取消、缓存和
  批量结果语义均由契约与真实库认证保护。
- 版本基线：v1.0.0 冻结在提交 `f22c9f4`；v1.0.1 从该基线创建 `1.0.1` 分支，后续规划和实现不回写冻结基线。
- 详细规划：`docs/v1.0.1-roadmap.md`。
...
- 详细规划：`docs/v1.0.1-roadmap.md` 阶段 7。
...
- 状态：V1.0.1 框架无关日志策略、上层 YAML 映射、真实库与性能回归均已完成
...
- 详细规划：`docs/v1.0.1-roadmap.md` 阶段 8。

## R-031 V2.0.0 JDBC/R2DBC 双执行内核与统一接入

- 状态：V2 代码能力和发布门禁已完成；MySQL、PostgreSQL、Oracle、SQL Server 已完成真实数据库认证
- 来源：用户要求在全部 V1.x 能力之上真正接入 JDBC，同时保持所有业务入口统一，并能在 JDBC 与 R2DBC 之间低成本切换。
...
- 执行边界：V2 删除 V1 的 R2DBC 阻塞同步桥接。没有 `DataSource` 就不装配同步能力，没有 `ConnectionFactory` 就不装配
