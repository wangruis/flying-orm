package com.flying.orm.core.condition;

import com.flying.orm.core.field.FieldIdentity;
import com.flying.orm.core.internal.Names;
import com.flying.orm.core.internal.value.BindableValueSnapshots;

import java.util.ArrayList;
import java.util.List;
import java.util.Objects;

/**
 * 单个参数驱动条件，保留字段、term id 和参数值，不在条件层提前拼接 SQL。
 *
 * @param identity 字段身份
 * @param operator term id，例如 `=`、`like`、`user-in-org`
 * @param value    条件参数值
 * <p>标准多值 term（`in`、`not-in`、`between`、`not-between`）会在构造时取得集合快照，
 * 集合中的直接数组元素和顶层数组都会独立复制。可信 direct-AST 自定义 term 的非数组值保持原对象，由其处理器定义不可变性边界，
 * 不能在条件层按 Java 容器接口猜测业务或驱动值的语义。</p>
 * @author wangr
 * @date 2026-08-09
 * @version v1.0
 */
public record TermCondition(FieldIdentity identity, String operator, Object value) implements ConditionNode {

    /**
     * 创建 term 条件并完成字段名和 term id 的基础规范化。
     *
     * @param identity 字段身份
     * @param operator term id
     * @param value    条件参数值
     */
    public TermCondition {
        identity = Objects.requireNonNull(identity, "condition field identity must not be null");
        if (value instanceof OwnedValue owned) {
            operator = Objects.requireNonNull(operator, "condition operator must not be null");
            value = owned.value();
        } else {
            operator = Names.key(operator, "condition operator");
            value = snapshotValue(operator, value);
        }
    }

    /**
     * 由字段名创建条件；名称在进入 AST 时只规范化一次。
     *
     * @param field    字段名或属性名
     * @param operator term id
     * @param value    条件参数值
     */
    public TermCondition(String field, String operator, Object value) {
        this(FieldIdentity.of(field), operator, value);
    }

    /** @return 保留声明大小写的字段名 */
    public String field() {
        return identity.name();
    }

    /**
     * 返回条件参数。数组可达图按次返回独立副本，避免调用方通过 record 访问器改写已经发布的条件树。
     *
     * @return 标准多值集合及其中数组可达图的只读快照、顶层数组可达图的独立副本；
     * 自定义 direct-AST term 的非数组值原样返回
     */
    @Override
    public Object value() {
        if (requiresCollectionSnapshot(operator) && value instanceof List<?> values) {
            return BindableValueSnapshots.logicalValues(values);
        }
        return isStandardOperator(operator)
                ? BindableValueSnapshots.logicalValue(value)
                : BindableValueSnapshots.arrayGraph(value);
    }

    /** 仅供构造期可信执行视图读取已经取得所有权的值。 */
    Object ownedValue() {
        return value;
    }

    /**
     * 创建 term 条件。
     *
     * @param field    字段名或属性名
     * @param operator term id
     * @param value    条件参数值
     * @return term 条件
     */
    public static TermCondition of(String field, String operator, Object value) {
        return new TermCondition(field, operator, value);
    }

    /**
     * 复用已有字段身份创建条件。
     *
     * @param identity 字段身份
     * @param operator term id
     * @param value    条件参数值
     * @return term 条件
     */
    public static TermCondition of(FieldIdentity identity, String operator, Object value) {
        return new TermCondition(identity, operator, value);
    }

    /** 内部编译链发布已经完成值规范化、不会再修改的条件。 */
    static TermCondition owned(FieldIdentity identity, String operator, Object value) {
        return new TermCondition(identity, operator, new OwnedValue(value));
    }

    /** 在条件入口的单次规范化中取得可变标量所有权，避免发布 AST 时再次遍历集合。 */
    static Object snapshotScalar(String operator, Object value) {
        return isStandardOperator(operator)
                ? BindableValueSnapshots.logicalValue(value)
                : BindableValueSnapshots.arrayGraph(value);
    }

    private static Object snapshotValue(String operator, Object value) {
        // 直接 AST 的自定义 term 只有处理器自己知道值形状；不能因其实现 Collection 就改写驱动标量的类型。
        // 因而仅对内置多值 term 做集合快照，数组仍一律按值复制以阻断发布后的元素改写。
        if (requiresCollectionSnapshot(operator)) {
            if (value instanceof Iterable<?> iterable) {
                return snapshotIterable(iterable);
            }
        }
        return snapshotScalar(operator, value);
    }

    /**
     * 标准多值 term 只在发布 AST 时消费一次 Iterable；既防止后续可变源改写 SQL，也不重复遍历一次性数据源。
     */
    private static List<Object> snapshotIterable(Iterable<?> values) {
        List<Object> snapshot = new ArrayList<>();
        for (Object item : values) {
            snapshot.add(item);
        }
        return BindableValueSnapshots.logicalValues(snapshot);
    }

    private static boolean requiresCollectionSnapshot(String operator) {
        return TermRegistry.standard()
                           .find(operator)
                           .map(TermHandler::shape)
                           .map(shape -> shape == ConditionValueShape.COLLECTION
                                   || shape == ConditionValueShape.RANGE)
                           .orElse(false);
    }

    private static boolean isStandardOperator(String operator) {
        return TermRegistry.standard().find(operator).isPresent();
    }

    private record OwnedValue(Object value) {
    }
}
