 ...
 *
 * @param identity   字段身份；保留 SQL 声明名称并缓存大小写无关查找键
 ...
 * @param precision  数值字段总有效位数，或时间字段小数秒位数；时间字段允许为 0，未指定时为空
 ...
 * @version v1.0
 */
public record DynamicField(FieldIdentity identity,
     ...
     */
    public DynamicField {
        identity = Objects.requireNonNull(identity, "dynamic field identity must not be null");
        databaseType = Objects.requireNonNull(databaseType, "dynamic field database type must not be null");
        nullable = !primaryKey && nullable;
        length = requirePositive(length, "dynamic field length");
        precision = requirePrecision(precision, databaseType, "dynamic field precision");
        scale = requireNonNegative(scale, "dynamic field scale");
        if (scale != null && databaseType.logicalType().temporal()) {
            throw new IllegalArgumentException("dynamic field scale must not be set for temporal data type");
        }
    ...
    public DynamicField(String name,
                        ...
                        String comment,
                        ValueGeneration generation) {
        this(FieldIdentity.of(name), DatabaseType.of(dataType), primaryKey, nullable, unique,
             length, precision, scale, comment, generation);
    }

    /** Internal constructor that reuses an already parsed field identity and type. */
    public DynamicField(FieldIdentity identity,
                        String dataType,
                        boolean primaryKey,
                        boolean nullable,
                        boolean unique,
                        ...
                        String comment,
                        ValueGeneration generation) {
        this(identity, DatabaseType.of(dataType), primaryKey, nullable, unique,
             length, precision, scale, comment, generation);
    }

    /** Creates a field from a public name while reusing an already parsed database type. */
    public DynamicField(String name,
                        DatabaseType databaseType,
                        boolean primaryKey,
                        boolean nullable,
                        ...
                        String comment,
                        ValueGeneration generation) {
        this(FieldIdentity.of(name), databaseType, primaryKey, nullable, unique,
             length, precision, scale, comment, generation);
    }

    /** @return 保留大小写的字段声明名称，供方言标识符渲染 */
    public String name() {
        return identity.name();
    }

    /** @return database type declaration retained for SQL rendering and public metadata */
    ...
    /** Creates a regular field while reusing an already parsed database type. */
    public static DynamicField of(String name, DatabaseType databaseType) {
        return new DynamicField(name, databaseType, false, true, false,
                                null, null, null, null, ValueGeneration.none());
    }
    ...
    /** Creates a primary-key field while reusing an already parsed database type. */
    public static DynamicField primaryKey(String name, DatabaseType databaseType) {
        return new DynamicField(name, databaseType, true, false, false,
                                null, null, null, null, ValueGeneration.none());
    }
    ...
    public DynamicField withNullable(boolean nullable) {
        return new DynamicField(identity, databaseType, primaryKey, nullable, unique,
                                length, precision, scale, comment, generation);
    }
    ...
    /** @return 仅替换数值精度/小数位或时间小数秒精度的新字段定义 */
    ...
    public DynamicField withGeneration(ValueGeneration generation) {
        return new DynamicField(identity,
                                databaseType,
                                primaryKey,
                                nullable,
                                unique,
