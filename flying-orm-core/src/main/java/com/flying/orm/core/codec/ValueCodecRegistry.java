...
public final class ValueCodecRegistry {
    ...
    public <T> T read(Object value, Class<T> targetType) {
        ...
        return safeTargetType.isInstance(adapted)
                ? safeTargetType.cast(adapted)
                : readWithCodec(adapted, safeTargetType);
    }
    ...
    private <T> T readWithCodec(Object value, Class<T> targetType) {
        return (T) find(targetType).read(value, targetType);
    }

    private ValueCodec find(Class<?> targetType) {
        // 第一个匹配项获胜，扩展注册表时具体类型必须排在通用类型前。
        return codecs.stream()
                     .filter(codec -> codec.supports(targetType))
                     .findFirst()
                     .orElseThrow(() -> new IllegalArgumentException("no value codec for " + targetType.getName()));
    }
}
