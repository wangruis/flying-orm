...
public final class FieldScope {
    ...
    public boolean canRead(String field) {
        return unrestrictedRead || readableFields.contains(normalize(field, "field scope lookup"));
    }

    /** 判断字段是否允许出现在 insert/update 数据中，比较时忽略大小写和首尾空白。 */
    public boolean canWrite(String field) {
        return unrestrictedWrite || writableFields.contains(normalize(field, "field scope lookup"));
    }
    ...
    private static Set<String> normalize(Set<String> fields, String fieldName) {
        ...
        for (String field : fields) {
            values.add(normalize(field, fieldName));
        }
        return Collections.unmodifiableSet(values);
    }
    ...
    private static Set<String> fields(String first, String... rest) {
        ...
        fields.add(first);
        if (rest != null) {
            fields.addAll(java.util.List.of(rest));
        }
        return fields;
    }
