 ...
 * 批量观测模型的状态归类和统计工具。
 *
 * <p>这些规则只服务于同包内的事件工厂，不作为公共扩展点。把它们从公共 record 中拿出来，
...
final class BatchObservationClassification {
    ...
    static long successfulChunkCount(BatchExecutionEventType eventType, BatchChunkResult.Status status) {
        return eventType == BatchExecutionEventType.CHUNK && isSuccessful(status) ? 1 : 0;
    }

    static long failedChunkCount(BatchExecutionEventType eventType, BatchChunkResult.Status status) {
        return eventType == BatchExecutionEventType.CHUNK && !isSuccessful(status) ? 1 : 0;
    }
