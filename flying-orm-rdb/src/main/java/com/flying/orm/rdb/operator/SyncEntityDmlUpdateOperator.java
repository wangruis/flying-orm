...
public final class SyncEntityDmlUpdateOperator<T> {
    ...
    /** 使用已注册条件运算符追加 AND 条件。 */
    public SyncEntityDmlUpdateOperator<T> and(EntityProperty<T, ?> property, String operator, Object value) { return where(property, operator, value); }
    /** 以 AND 追加标准或已注册的业务条件，字段和值均不作为原始 SQL。 */
    public SyncEntityDmlUpdateOperator<T> where(EntityProperty<T, ?> property, String operator, Object value) { command.state().where().where(property, operator, value); return this; }
    /** 添加 IS NULL 条件。 */
