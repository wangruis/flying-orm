...
final class JdbcBatchChunkExecutor {
    ...
    BatchChunkResult execute(Connection connection,
        ...
        List<Object[]> safeRows = Objects.requireNonNull(rows, "jdbc batch rows must not be null");
        if (safeRows.isEmpty()) {
        ...
        try (PreparedStatement statement = safeConnection.prepareStatement(safeRequest.sql())) {
            ...
            for (Object[] row : safeRows) {
                validateRow(safeRequest, row);
                JdbcStatementBinder.bind(statement, Arrays.asList(
                        ProtectedBatchRows.parameters(row, safeRequest.parameterCount())));
                statement.addBatch();
                deadline.remaining();
            }
            deadline.remaining();
    ...
    private static BatchChunkResult executeReturningGeneratedKeys(Connection connection,
        ...
        try (PreparedStatement statement = connection.prepareStatement(
            ...
            for (int index = 0; index < rows.size(); index++) {
                ...
                JdbcStatementBinder.bind(statement, Arrays.asList(
                        ProtectedBatchRows.parameters(row, request.parameterCount())));
    ...
    private static BatchChunkResult executeOwnerRestrictedUpdates(
        ...
        for (int index = 0; index < rows.size(); index++) {
            ...
            long count;
            if (work == null) {
                count = executeUpdate(connection, new SqlRequest(
                        request.sql(), Arrays.asList(ProtectedBatchRows.parameters(
                                rows.get(index), request.parameterCount())), request.bindMarkerStyle()), deadline);
            } else if (work.kind() != ProtectedWriteWork.Kind.UPDATE) {
                throw new IllegalArgumentException("protected batch work kind does not match update request");
            } else if (state.owners().isEmpty()) {
                count = 0L;
            } else {
                count = executeUpdate(connection, work.writeRequestForOwners(state.owners()), deadline);
            }
            collectRowCount(request, startOffset + index, count, conflicts);
            if (count > 0L) {
                affectedRows = addExact(affectedRows, count);
    ...
    private static long executeUpdate(Connection connection,
                                      SqlRequest request,
                                      JdbcBatchSupport.BatchDeadline deadline) throws SQLException, TimeoutException {
        deadline.remaining();
    ...
    private static void validateRow(BatchWriteRequest request, Object[] row) {
