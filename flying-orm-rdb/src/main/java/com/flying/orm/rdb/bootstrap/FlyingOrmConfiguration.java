 ...
 * 配置系统绑定到这份不可变数据，再交给 {@link FlyingOrmBootstrap}。这样不同框架不会各自解释容量、批量、缓存
 * 或日志默认值，也不会在适配层复制 ORM 装配逻辑。</p>
 *
 * <p>{@code dialect} 必须由上层显式选择；默认配置暂未指定，完成装配前需要补充，不进行自动探测。
 * SQL 日志默认关闭，完整 SQL 和参数仍由 {@link SqlExecutionLogOptions} 单独控制。</p>
 *
 * <p>执行选项只保留容量限制；时间政策由上层治理。
 * 批量按有界缓冲顺序执行，只报告 SQL 执行事实。</p>
 *
 * @param dialect                  显式方言名；未指定时不能直接完成数据库客户端装配
 * @param executionOptions         普通 SQL 默认执行保护
 * @param batchWriteOptions        默认批量输入容量策略
...
public record FlyingOrmConfiguration(String dialect,
    ...
    /** 返回显式指定方言的新配置；空白文本表示尚未指定，不启用自动识别。 */
