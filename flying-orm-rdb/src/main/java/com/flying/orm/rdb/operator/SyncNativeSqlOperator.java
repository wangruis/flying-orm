...
public final class SyncNativeSqlOperator {
    ...
    public DynamicRow one() {
        return SyncSqlResultOperations.one(query(), "native SQL one()");
    }

    /** 查询并映射最多一行，空结果返回 null。 */
    public <T> T one(Class<T> type) {
        return SyncSqlResultOperations.one(query(type), "native SQL one()");
    }

    /** 使用自定义映射器查询最多一行，空结果返回 null。 */
    public <T> T one(RowMapper<T> mapper) {
        return SyncSqlResultOperations.one(query(mapper), "native SQL one()");
    }
