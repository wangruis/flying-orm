...
public final class NativeSqlOperator {
    ...
    /**
     * 绑定带明确 Java 类型的空值，向驱动提供空值类型提示。
     * 某些驱动在原生 SQL 完全没有类型上下文时仍要求 SQL 显式转换，
     * 例如 PostgreSQL 的 {@code :value is null} 可写为 {@code cast(:value as timestamp) is null}。
     *
     * @param name SQL 中 {@code :name} 的名称，不包含冒号
     * @param javaType 驱动绑定空值时使用的非原始 Java 类型
     * @return 当前构建器
     */
    public NativeSqlOperator bindNull(String name, Class<?> javaType) {
    ...
    /**
     * 给本次执行设置最大返回行数、结果内存和 LOB 上限等保护。没有调用时沿用执行器的默认保护。
     *
