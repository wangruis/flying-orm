...
final class FormQueryShapeGuard {
    ...
    static void requireCursorProjection(List<String> projections, List<CursorSort> sorts) {
        ...
        for (CursorSort sort : sorts) {
            if (!containsIgnoreCase(projections, sort.field())) {
    ...
    private static boolean containsIgnoreCase(List<String> fields, String candidate) {
        return fields.stream().anyMatch(field -> field.equalsIgnoreCase(candidate));
    }
}
