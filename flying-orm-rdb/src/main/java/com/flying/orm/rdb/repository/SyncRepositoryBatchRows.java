...
/**
 * 为同步批量输入组合 PRE 回调、实体映射和可回收的输入偏移。
 *
...
final class SyncRepositoryBatchRows<T, R> implements Publisher<R> {
    ...
    SyncRepositoryBatchRows(Publisher<T> source,
        ...
        this.source = Objects.requireNonNull(lifecycle, "repository lifecycle support must not be null")
                .beforeRows(Objects.requireNonNull(source, "repository batch source must not be null"),
                            Objects.requireNonNull(beforePhase, "batch before lifecycle phase must not be null"));
    ...
    /** 无监听器、无生成键时只保留 Reactive Streams 映射，不进入生命周期 tracker。 */
    SyncRepositoryBatchRows(Publisher<T> source, Function<T, R> mapper) {
