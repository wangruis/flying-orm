 ...
 * @version v1.0
 */
public final class JsonStructuredConditions implements StructuredConditionResolver, StructuredConditionCustomizer {
    ...
    private JsonConditionValue containsValue(Object value) {
        ...
        if (value == null) {
            throw new IllegalArgumentException("json-contains value is required");
        }
        // 条件可能在本次请求的异步下游才渲染；保存规范 JSON 文本以冻结当前输入，不能继续持有 caller 容器。
        return JsonConditionValue.contains(JsonValueCodec.write(value));
    }
