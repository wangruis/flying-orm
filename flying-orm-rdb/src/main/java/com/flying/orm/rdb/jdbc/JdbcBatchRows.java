...
final class JdbcBatchRows implements Subscriber<Object[]>, AutoCloseable {
    ...
    JdbcBatchRows(Publisher<Object[]> publisher) {
        this(publisher, -1, Long.MAX_VALUE);
    }

    JdbcBatchRows(Publisher<Object[]> publisher, int parameterCount, long maxBufferedBytes) {
        this.publisher = Objects.requireNonNull(publisher, "batch row publisher must not be null");
        if (parameterCount < -1 || maxBufferedBytes <= 0L) {
    ...
    Object[] next(Duration remaining) throws InterruptedException, TimeoutException {
        startIfNeeded();
        long remainingNanos = remaining.isZero() ? Long.MAX_VALUE : saturatingNanos(remaining);
        while (true) {
    ...
    public void onNext(Object[] row) {
        ...
        try {
            ...
            if (!requested || nextRow != null || row == null) {
                ...
                requested = false;
                try {
                    // 上游可以在 onNext 返回后立刻复用全部嵌套载荷；所有权快照必须在接收信号内完成。
                    nextRow = parameterCount < 0
                            ? BatchRowSnapshotter.snapshot(row)
                            : BatchRowSnapshotter.snapshot(
                                    row, parameterCount, maxBufferedBytes, "buffered bytes");
    ...
    private void cancelAfterFailure(Subscription cancel, Throwable primary) {
        try {
            ...
            VirtualMachineError cleanupFatal = JdbcThrowableGraph.findVirtualMachineError(cleanup);
            ...
            if (secondary != null && secondary != selected) {
                JdbcThrowableGraph.addSuppressedIfAcyclic(selected, secondary);
            }
            lock.lock();
            try {
                failure = selected;
