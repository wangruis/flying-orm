...
public interface PaginationDialect {
    ...
    /** 内核只取 SQL 时不构造内置方言的临时请求；外部扩展仍执行原 paginate 回调。 */
    @InternalApi
    static String paginateSql(PaginationDialect dialect, String sql, List<Object> parameters, PageQuery page) {
        if (dialect instanceof StandardPaginationDialect standard) {
            Objects.requireNonNull(page, "page query must not be null");
            return standard.paginateSql(sql);
        }
        return dialect.paginate(sql, parameters, page).sql();
    }
...
enum StandardPaginationDialect implements PaginationDialect {
    ...
    LIMIT_OFFSET {
        @Override
        String paginateSql(String sql) {
            return requireSql(sql) + " limit ? offset ?";
        }
    ...
    OFFSET_FETCH {
        @Override
        String paginateSql(String sql) {
            return requireSql(sql) + " offset ? rows fetch next ? rows only";
        }
    ...
    SQL_SERVER_OFFSET_FETCH {
        ...
        String paginateSql(String sql) {
            ...
            return safeSql + " offset ? rows fetch next ? rows only";
        }
    ...
    private static final SqlLexicalScanner.Rules SQL_SERVER_LEXICAL_RULES =
            SqlLexicalScanner.rulesFor("sqlserver");

    abstract String paginateSql(String sql);

    @Override
    public SqlRequest paginate(String sql, List<Object> parameters, PageQuery page) {
        PageQuery safePage = requirePage(page);
        return new SqlRequest(paginateSql(sql), paginationParameters(parameters, safePage));
    }
    ...
    private static boolean hasTopLevelOrderBy(String sql) {
        ...
        for (int index = 0; index < sql.length();) {
            long protectedSegment = SqlLexicalScanner.protectedSegmentAt(
                    sql, index, SQL_SERVER_LEXICAL_RULES, false);
