...
final class PostgreSqlReactiveFormMetadataReader implements ReactiveFormMetadataReader, ReactiveMetadataExecutorSource {
    ...
    private static String logicalType(String dataType) {
        ...
        if (type.endsWith("[]")) {
            ...
            // PostgreSQL 数组按元素类型精确匹配运算符；int2[] 不能折叠成 integer[]。
            if ("smallint".equals(elementType) || "int2".equals(elementType)) {
                return "SMALLINT[]";
            }
            return logicalType(elementType) + "[]";
        }
        return switch (type) {
            ...
            case "timestamp without time zone", "timestamp with time zone", "timestamp" -> "TIMESTAMP";
            case "date" -> "DATE";
