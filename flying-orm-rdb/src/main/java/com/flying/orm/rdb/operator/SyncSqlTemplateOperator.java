...
public final class SyncSqlTemplateOperator {
    ...
    SyncSqlTemplateOperator(SyncSqlExecutor executor,
        ...
        this.state = new SqlTemplateExecutionState(engine, templateId, serverParameters);
    }
    ...
    /**
     * 设置结果容量保护；所有时间政策由上层或基础设施治理。
     *
     * @param options 最大返回行数、结果内存和 LOB 上限等保护选项
     * @return 当前单次调用构建器
     */
    ...
    public List<DynamicRow> query() {
        ...
        return SyncSqlResultOperations.query(
                executor, render(snapshot), snapshot.options());
    }
    ...
    public DynamicRow one() {
        ...
        return SyncSqlResultOperations.one(
                executor, render(snapshot), snapshot.options(), "SQL template one()");
    }
    ...
    public <T> T one(RowMapper<T> mapper) {
        return SyncSqlResultOperations.one(mappedQuery(mapper, 2), "SQL template one()");
    }

    private <T> List<T> mappedQuery(RowMapper<T> mapper, int rowLimit) {
        SqlTemplateExecutionState.Snapshot snapshot = state.snapshot();
        return SyncSqlResultOperations.queryMapped(
                executor, render(snapshot), snapshot.options(), mapper, rowLimit);
    }

    private com.flying.orm.core.sql.render.SqlRequest render(SqlTemplateExecutionState.Snapshot snapshot) {
        ...
        return state.render(snapshot, serverValues);
    }
}
