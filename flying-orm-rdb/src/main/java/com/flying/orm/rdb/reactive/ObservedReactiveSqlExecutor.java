...
final class ObservedReactiveSqlExecutor extends ForwardingReactiveSqlExecutor {
    ...
    private ObservedReactiveSqlExecutor(ReactiveSqlExecutor delegate,
        ...
        this.observationSupport = ReactiveSqlExecutionObservationSupport.create(
                this.observer, this.batchObserver);
    }
    ...
    /** 保护写入作为一个同连接工作单元观测，侧索引内部参数不单独暴露。 */
    @Override
    public Mono<SqlWriteResult> protectedWrite(ProtectedWriteWork work, SqlExecutionOptions options) {
        ...
        Mono<SqlWriteResult> source = Mono.defer(() -> delegate().protectedWrite(safeWork, safeOptions)
                .onErrorMap(ReactiveSqlExecutionProtection::translate));
    ...
    public Mono<BatchExecutionEvidence> writeBatch(BatchWriteRequest request,
        ...
        return observationSupport.observeBatchResult(request,
                Mono.defer(() -> delegate().writeBatch(request, rowCompleted)));
    }
    ...
    public Mono<BatchExecutionEvidence> writeProtectedBatch(BatchWriteRequest request,
        ...
        Objects.requireNonNull(request, "batch request must not be null");
        return observationSupport.observeBatchResult(request,
                Mono.defer(() -> delegate().writeProtectedBatch(request, rowCompleted)));
    }
}
