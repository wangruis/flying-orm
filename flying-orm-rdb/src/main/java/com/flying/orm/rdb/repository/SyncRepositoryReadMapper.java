...
final class SyncRepositoryReadMapper<T> {
    ...
    PageResult<T> page(ConditionGroup where, PageQuery page, DataScope scope, SqlExecutionOptions options) {
        QuerySpec query = EntityQueryDefaults.query(
                form, metadata, where, scope, options, client.defaultFieldScope());
        if (!page.sorts().isEmpty()) {
            query = query.withSorts(page.sorts());
        }
        return lifecycle.postLoad(client.page(query, page, entityType));
    }
}
