...
final class R2dbcParameterValues {
    ...
    static Object[] stabilizeBatchParameters(Object[] row, int parameterCount) {
        Objects.requireNonNull(row, "row");
        int boundCount = Math.min(parameterCount, row.length);
        for (int index = 0; index < boundCount; index++) {
            if (row[index] instanceof Parameter) {
                row[index] = snapshotForExecution(row[index]);
            }
        }
        return row;
    }
    ...
    private static Object forBinding(Object value, boolean owned) {
        ...
        if (value instanceof Parameter parameter) {
            ...
            Object bindingValue = parameterValue instanceof ByteBuffer buffer
                    ? buffer.asReadOnlyBuffer() : parameterValue;
            return stableParameter(parameter, parameterType, bindingValue);
        }
        if (value instanceof ByteBuffer buffer) {
            // Drivers may advance the supplied cursor while encoding. A cold execution can be
            // subscribed more than once, so every bind gets its own view over the owned bytes.
