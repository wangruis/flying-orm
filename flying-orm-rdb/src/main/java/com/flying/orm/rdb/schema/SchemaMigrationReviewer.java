...
final class SchemaMigrationReviewer {
    ...
    private SchemaRollbackPlan rollbackExisting(TableMetadata current,
        ...
        for (DynamicField target : migration.target().fields()) {
            ...
            current.findColumn(sourceName == null ? target.name() : sourceName)
                   .ifPresent(column -> targetsByCurrentName.put(column.name(), target));
        }
        ...
        for (ColumnMetadata source : reversedCurrent) {
            ...
            if (target == null) {
                ...
                if (source.primaryKey() != target.primaryKey()) {
                    ...
                    if (SchemaMigrationSupport.safeWidening(
                            source, target, renderer.tableRenderer())) {
                        gaps.add(new SchemaRollbackGap(
                                SchemaRollbackGap.Kind.DATA_CANNOT_BE_RESTORED,
                                source.name(),
                                "values written after widening must fit the original type before rollback"));
                    }
                }
            }
        ...
        List<DynamicField> retainedFields = new ArrayList<>();
        ...
        List<DynamicField> rollbackSequenceOwners = new ArrayList<>(retainedFields);
        current.columns().stream()
               .map(SchemaMigrationSupport::toDynamicField)
               .forEach(rollbackSequenceOwners::add);
        rollback.addAll(renderer.rollbackDropSequences(addedFields, rollbackSequenceOwners));
        ...
        if (hasForeignKeyChange(migration)) {
    ...
    private static boolean skipped(SchemaMigrationPlan migration,
        ...
        return migration.skippedChanges().stream()
                        .anyMatch(change -> change.kind() == kind && change.name().equals(name));
    }

    private static boolean hasForeignKeyChange(SchemaMigrationPlan migration) {
        return migration.skippedChanges().stream()
                        .map(SkippedSchemaChange::kind)
                        .anyMatch(kind -> kind == SkippedSchemaChange.Kind.ADD_FOREIGN_KEY
                                || kind == SkippedSchemaChange.Kind.DROP_FOREIGN_KEY
                                || kind == SkippedSchemaChange.Kind.CHANGE_FOREIGN_KEY);
    }
    ...
    private boolean hasColumnCommentChange(SchemaMigrationPlan migration,
        ...
        String expected = renderer.dialect()
                                  .columnCommentChangeSql(table, target.name(), sourceComment, targetComment)
                                  .orElse(null);
        if (expected == null
                && renderer.dialect().inlineColumnComment()
                && renderer.dialect().rewritesFullColumnDefinition()
                && sameShape(source, target)) {
    ...
    private static String renameTarget(Map<String, String> renames, String source) {
        return SchemaMigrationSupport.renameTargetForSource(renames, source);
    }

    private static String renameSource(Map<String, String> renames, String target) {
        return SchemaMigrationSupport.renameSourceForTarget(renames, target);
    }
}
