...
public final class JdbcAdvancedOperations {
    ...
    public <T> T scroll(SqlRequest request,
        ...
        try {
            ...
            T result = executeScrollable(lease, safeRequest, safeOptions, safeCallback);
            observations.success(SqlExecutionOperation.QUERY, safeRequest, 0L, startedAt, source);
            return result;
        } catch (SQLException | RuntimeException error) {
            RuntimeException translated = RdbExceptionTranslator.translate(error);
            observations.failure(SqlExecutionOperation.QUERY, safeRequest, 0L, startedAt, translated, source);
            throw translated;
        }
    }
    /**
                              ...
                              lease -> lease.externalTransaction() == null
                                      ? lease.connection().getMetaData()
                                       : JdbcExternalTransactionConnectionView.metadata(
                                               lease.connection().getMetaData(),
                                               JdbcExternalTransactionConnectionView.connection(lease.connection())));
    }
    /**
    ...
    private <S, T> T withConnection(JdbcCallback<S, T> callback,
        ...
        try {
            ...
            if (lease != null) {
                discardConnectionFailure(lease, failure);
    ...
    private static void closeConnectionLease(JdbcConnectionProvider.JdbcConnectionLease lease,
        ...
        try {
            ...
            VirtualMachineError failureFatal = JdbcThrowableGraph.findVirtualMachineError(failure);
            if (failureFatal != null) {
                JdbcThrowableGraph.addSuppressedIfAcyclic(failureFatal, cleanupError);
                throw failureFatal;
            }
            VirtualMachineError cleanupFatal = JdbcThrowableGraph.findVirtualMachineError(cleanupError);
            if (cleanupFatal != null) {
                JdbcThrowableGraph.addSuppressedIfAcyclic(cleanupFatal, failure);
                throw cleanupFatal;
            }
            if (failure != null) {
                JdbcThrowableGraph.addSuppressedIfAcyclic(failure, cleanupError);
                return;
            }
    ...
    private <T> T executeScrollable(JdbcConnectionProvider.JdbcConnectionLease lease,
        ...
        try {
            ...
            ResultSet callbackResultSet = lease.externalTransaction() == null
                    ? resultSet
                    : JdbcExternalTransactionConnectionView.resultSet(
                            resultSet, JdbcExternalTransactionConnectionView.connection(lease.connection()));
