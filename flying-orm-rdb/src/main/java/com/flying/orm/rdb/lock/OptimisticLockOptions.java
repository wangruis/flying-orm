...
public record OptimisticLockOptions(String field,
    ...
    /**
     * 返回构造时固定的旧版本值；数组值每次都返回完整数组图副本，避免调用方改写后续 WHERE 参数。
     *
     * @return 旧版本值；非数组保留原对象引用，数组返回新的数组图副本
     */
    @Override
    ...
    /**
     * 返回构造时固定的新版本值；仅 ASSIGN 模式有值，数组值每次都返回完整数组图副本。
     *
     * @return 新版本值；INCREMENT 模式为 {@code null}，非数组保留原对象引用，数组返回新的数组图副本
     */
    @Override
    ...
    private static Object snapshot(Object value) {
        return MutableValueSnapshots.arrayGraph(value);
    }
}
