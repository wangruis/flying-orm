...
public final class ProtectedValueNormalizerRegistry {
    ...
    /**
     * 执行规范化并验证结果非空且不超过字段边界。确定性由扩展契约保证。
     *
     * @param id              规范化器 ID
     * @param value           原始业务文本
    ...
    public String normalize(String id, String value, int maxCodePoints) {
        ...
        String normalized = normalize(normalizer, input);
        if (normalized == null) {
            throw new IllegalArgumentException("protected value normalizer must not return null");
        }
