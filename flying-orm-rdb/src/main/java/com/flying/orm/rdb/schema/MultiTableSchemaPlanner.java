...
public final class MultiTableSchemaPlanner {
    ...
    public Plan plan(RelationalSchemaDefinition desired) {
        ...
        requireValidSchemaConstraintNamespace(safeDesired);
    ...
    private void requireValidSchemaConstraintNamespace(RelationalSchemaDefinition schema) {
        List<String> constraintNames = new ArrayList<>();
        List<String> indexNames = new ArrayList<>();
        switch (database.dialectId()) {
            case "mysql" -> schema.tables().forEach(table -> {
                table.checks().forEach(check -> constraintNames.add(check.name()));
                table.foreignKeys().forEach(foreignKey -> constraintNames.add(foreignKey.name()));
            });
            case "oracle" -> schema.tables().forEach(table -> {
                table.primaryKey().ifPresent(primaryKey -> constraintNames.add(primaryKey.name()));
                table.uniqueConstraints().forEach(unique -> constraintNames.add(unique.name()));
                table.checks().forEach(check -> constraintNames.add(check.name()));
                table.foreignKeys().forEach(foreignKey -> constraintNames.add(foreignKey.name()));
                table.indexes().forEach(index -> indexNames.add(index.name()));
            });
            case "postgresql" -> schema.tables().forEach(table -> {
                table.primaryKey().ifPresent(primaryKey -> indexNames.add(primaryKey.name()));
                table.uniqueConstraints().forEach(unique -> indexNames.add(unique.name()));
                table.indexes().forEach(index -> indexNames.add(index.name()));
            });
            case "sqlserver" -> schema.tables().forEach(table -> {
                table.primaryKey().ifPresent(primaryKey -> constraintNames.add(primaryKey.name()));
                table.uniqueConstraints().forEach(unique -> constraintNames.add(unique.name()));
                table.checks().forEach(check -> constraintNames.add(check.name()));
                table.foreignKeys().forEach(foreignKey -> constraintNames.add(foreignKey.name()));
        ...
        boolean exactNames = "postgresql".equals(database.dialectId())
                || "oracle".equals(database.dialectId());
        requireNoSchemaNameCollisions(constraintNames, "constraint", exactNames);
        requireNoSchemaNameCollisions(indexNames, "index", exactNames);
    }

    private static void requireNoSchemaNameCollisions(List<String> names,
                                                      String objectKind,
                                                      boolean exactNames) {
        Set<String> normalized = new HashSet<>();
        for (String name : names) {
            String identity = exactNames ? name : name.toLowerCase(Locale.ROOT);
            if (!normalized.add(identity)) {
                throw new IllegalArgumentException(
                        "relational " + objectKind + " names collide in the database schema namespace");
            }
        }
    }
