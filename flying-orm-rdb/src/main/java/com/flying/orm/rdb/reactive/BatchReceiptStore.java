...
final class BatchReceiptStore {
    ...
    Mono<Receipt> find(BatchChunkResult.RecoveryToken token, Duration timeout) {
        ...
        Flux<Receipt> receipts = Flux.usingWhen(acquireConnection(),
                                                ...
                                                this::closeAfterUnconfirmedRead,
                                                connection -> closeAfterUnconfirmedRead(
                                                        connection,
    ...
    /**
     * 先按稳定操作编号和分片编号查已完成回执，计划和 payload 摘要由调用方按协议版本比对。
     * 这个入口让安全重试不必先把所有参数行收进 List。
     */
    Mono<Receipt> findOperation(BatchWriteOptions.Recovery recovery,
                                int chunkIndex) {
        return findOperation(recovery, chunkIndex, Duration.ZERO);
    }
    ...
    private Mono<Receipt> findOperation(String operationId,
                                        ...
                                        int chunkIndex,
                                        Duration timeout) {
        String sql = "select plan_hash, payload_hash, row_count, affected_rows from " + receiptTable(receiptTable)
                + " where operation_id = ? and chunk_index = ? and status = 'COMMITTED'";
        Flux<Receipt> receipts = Flux.usingWhen(acquireConnection(),
                                                connection -> protectRead(
                                                        Flux.from(bind(connection.createStatement(sqlForDriver(sql, 2)),
                                                                       List.of(operationId,
                                                                               chunkIndex)).execute())
                                                            .flatMap(result -> result.map(this::receiptWithIdentity)),
                                                        timeout),
                                                this::closeAfterConfirmedRead,
                                                this::closeAfterUnconfirmedRead,
                                                connection -> closeAfterUnconfirmedRead(
    ...
    private Receipt receiptWithIdentity(Row row, RowMetadata ignored) {
        String planHash = Objects.requireNonNull(row.get(0, String.class),
                                                  "batch receipt plan_hash must not be null");
        String payloadHash = Objects.requireNonNull(row.get(1, String.class),
                                                     "batch receipt payload_hash must not be null");
        return new Receipt(planHash,
                           payloadHash,
                           exactLong(row, 2, "row_count"),
                           exactLong(row, 3, "affected_rows"));
    }
    ...
    static Receipt requireMatching(BatchChunkResult.RecoveryToken token, Receipt receipt) {
        ...
        if (!safeReceipt.planHash().isEmpty() && !safeReceipt.planHash().equals(safeToken.planHash())) {
            throw new BatchReceiptIntegrityException("plan_hash", "match complete recovery evidence",
                                                     new IllegalStateException("plan hash mismatch"));
        }
    ...
    private Mono<Void> closeAfterConfirmedRead(Connection connection) {
        return Mono.defer(() -> Mono.from(connection.close()))
                   .onErrorResume(error -> reportCleanupFailure(
                           error, ResourceCleanupObservation.Phase.CONNECTION_CLOSE, true));
    }

    private Mono<Void> closeAfterUnconfirmedRead(Connection connection, Throwable cause) {
        R2dbcCleanupDeadline deadline = R2dbcCleanupDeadline.start(SqlExecutionOptions.DEFAULT_CLEANUP_TIMEOUT);
        return finishClose(
                deadline.protect(Mono.defer(() -> Mono.from(connection.close()))),
                cause,
                ResourceCleanupObservation.Phase.CONNECTION_CLOSE,
                false);
    }

    private Mono<Void> finishClose(Mono<Void> closeAttempt,
        ...
        return closeAttempt.onErrorResume(closeError -> {
            ...
            observeCleanup(phase, outcomeConfirmed, closeError);
            return Mono.empty();
        }).then(Mono.defer(() -> {
    ...
    private Mono<Void> reportCleanupFailure(Throwable error,
                                            ResourceCleanupObservation.Phase phase,
                                            boolean outcomeConfirmed) {
        VirtualMachineError fatal = findVirtualMachineError(error);
        observeCleanup(phase, outcomeConfirmed, fatal == null ? error : fatal);
        return fatal == null ? Mono.empty() : Mono.error(fatal);
    }
    ...
    /** 已提交回执里对重建结果有用的字段。 */
    record Receipt(String planHash, String payloadHash, long rowCount, long affectedRows) {
