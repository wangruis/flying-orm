...
public final class FlyingOrmBootstrap {
     ...
     * 完成启动校验并创建可共享的客户端对象图。
     *
     * <p>方法不会提前打开数据库连接。方言自动识别、物理数据源一致性校验以及具体连接的获取，
     * 都由选中的 Builder 和执行器按各自内核规则完成。</p>
     *
    ...
    public static FlyingOrmClients create(FlyingOrmConfiguration configuration,
        ...
        configureDialectValidation(builder, safeEnvironment);
    ...
    /** 分别校验两条执行内核的物理数据源，双内核不能只校验其中一条。 */
    private static void configureDialectValidation(FlyingOrmClientBuilder builder,
                                                   FlyingOrmEnvironment environment) {
        if (!environment.physicalJdbcDataSources().isEmpty()) {
            builder.validateJdbcDataSourceDialects(environment.physicalJdbcDataSources());
        }
        if (!environment.physicalReactiveDataSources().isEmpty()) {
            builder.validateReactiveDataSourceDialects(environment.physicalReactiveDataSources());
        }
    }
