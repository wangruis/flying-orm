...
public record ConditionStructurePlan(ConditionPlan plan,
    ...
    public ConditionStructurePlan {
        ...
        if (OwnedBindableValues.isPublished(safeParameters)) {
            parameters = OwnedBindableValues.requiresImmutableSnapshot(safeParameters)
                    ? BindableValueSnapshots.immutableValues(safeParameters)
                    : safeParameters;
        } else {
            parameters = BindableValueSnapshots.immutableValues(safeParameters);
        }
        ...
        parameters = OwnedBindableValues.publishOwned(parameters);
    }

    @Override
    public List<Object> parameters() {
        // 公共读取隔离可变值，内部最终 SqlRequest 复用已经建立的参数所有权。
        return OwnedBindableValues.defensiveView(parameters);
    }
}
