...
public final class SyncEntityDmlQueryOperator<T>
    ...
    /** 使用已注册条件运算符追加 AND 条件。 */
    public SyncEntityDmlQueryOperator<T> and(EntityProperty<T, ?> property, String operator, Object value) { return where(property, operator, value); }
    /** 以 AND 追加标准或已注册的业务条件，字段和值均不作为原始 SQL。 */
    public SyncEntityDmlQueryOperator<T> where(EntityProperty<T, ?> property, String operator, Object value) { command.state().where().where(property, operator, value); return this; }
    /** 设置本次查询的敏感字段显示方式。 */
    @Override
    ...
    public T one() { return client.selectOne(command.entitySpec(), command.state().metadata().type()); }
    /** 执行有界页码分页。 */
