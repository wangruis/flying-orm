 ...
 * 一次审核结构计划执行的不可变证据。
 *
 * <p>步骤执行尝试和数据库返回的更新数分别记录；{@link StepResult#sqlSent()} 不证明 SQL 已发送或生效。
 * 只有执行后重新读取结构并通过原兼容模式
 * 验证，整体状态才可以是 {@link SchemaExecutionStatus#SUCCESS}。该状态仅描述 SQL 执行和结构回读事实。</p>
 *
 * @author wangr
 * @version v3.2
 */
public final class SchemaExecutionReport {
    ...
    public static final class StepResult {
         ...
         * 是否已进入本步骤的 SQL 执行尝试。
         *
         * <p>true 不证明已取得连接、驱动已发送、服务器已接收或 SQL 已生效；获取连接失败也可能为 true。
         * false 表示本步骤未进入 SQL 执行尝试。此标记不表示事务结果，不作为重试或恢复裁决依据。</p>
         *
         * @return 本步骤是否已进入 SQL 执行尝试
         */
