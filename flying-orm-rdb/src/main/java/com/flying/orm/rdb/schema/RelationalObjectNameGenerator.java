...
public final class RelationalObjectNameGenerator {
    ...
    /** 校验点分限定对象名时，对数据库限制逐段应用，而不是限制整串限定身份。 */
    String qualifiedObject(String name) {
        String qualifiedName = Names.requireText(name, "qualified relational object name");
        for (String part : qualifiedName.split("\\.", -1)) {
            exact(part, "qualified relational object name part");
        }
