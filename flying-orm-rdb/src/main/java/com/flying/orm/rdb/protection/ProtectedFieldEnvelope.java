...
final class ProtectedFieldEnvelope {
    ...
    static Parsed parse(byte[] envelope) {
        try {
            ...
            if (versionLength < 1 || versionLength > 16 || source.remaining() < versionLength + NONCE_LENGTH + 4 + 16) {
                throw new ProtectedFieldException();
            }
            ...
            return new Parsed(new String(version, StandardCharsets.US_ASCII), nonce, ciphertext);
    ...
    static String keyVersion(byte[] envelope) {
        return parse(envelope).keyVersion();
    }
