...
final class RepositoryBatchLayoutPolicy {
    ...
    private static boolean hasVariableLayout(EntityFieldMetadata field, boolean upsert) {
        ...
        // readForUpsert 只要 INSERT 或冲突后的 UPDATE 任一阶段允许字段，就会保留该字段。
        // 任一阶段为 ALWAYS/DEFAULT 时列一定存在；否则只要存在条件策略，列就可能随实体值变化。
        boolean alwaysIncluded = (field.insertable() && isAlways(field.insertStrategy()))
                || (field.updatable() && isAlways(field.updateStrategy()));
        if (alwaysIncluded) {
            return false;
        }
    ...
    private static boolean isAlways(FieldStrategy strategy) {
        return strategy == FieldStrategy.DEFAULT || strategy == FieldStrategy.ALWAYS;
    }
