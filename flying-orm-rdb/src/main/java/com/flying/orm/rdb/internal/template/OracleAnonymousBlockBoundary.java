    ...
    private static final class BlockValidator {
        ...
        private void accept(SqlLexicalScanner.SegmentKind kind, int start, int end) {
            ...
            for (int index = start; index < end;) {
                ...
                if (completed) {
                    ...
                    if ("SUBPROGRAM_HEADER".equals(scopes.peek()) && parentheses == 0) {
                        scopes.pop();
                    }
                    endLabel = false;
        ...
        private void acceptWord(String word) {
            if (pendingEnd) {
                ...
                scopes.push("DECLARE");
            } else if (("PROCEDURE".equals(word) || "FUNCTION".equals(word))
                    && "DECLARE".equals(scopes.peek())) {
                scopes.push("SUBPROGRAM_HEADER");
            } else if (("IS".equals(word) || "AS".equals(word))
                    && "SUBPROGRAM_HEADER".equals(scopes.peek()) && parentheses == 0) {
                scopes.pop();
                scopes.push("DECLARE");
            } else if (isScopeStart(word)) {
                if ("BEGIN".equals(word) && "DECLARE".equals(scopes.peek())) {
                    // A local body consumes only its own declaration section, not the enclosing one.
                    scopes.pop();
                }
                scopes.push(word);
            }
