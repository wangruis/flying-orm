 ...
 *
 * <p>有界缓存按显式策略启用，不检测连接或事务状态。上层需要隔离的结构视图时使用独立 reader 或禁用缓存；
 * Schema 审核始终直读当前数据库事实。</p>
 *
 * @author wangr
 * @version v2.0.0
 */
public final class JdbcFormMetadataReader implements MetadataCacheInvalidator {
    ...
    /** 按 table 或 schema.table 读取 Schema 规划的完整表元数据，不命中也不写入普通 CRUD 缓存。 */
    public TableMetadata readTableForSchema(String table) {
        return loadTable(MetadataCacheKey.table(null, table));
    }
