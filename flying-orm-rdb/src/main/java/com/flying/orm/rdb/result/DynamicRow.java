...
public final class DynamicRow extends AbstractMap<String, Object> {
    ...
    /**
     * 返回当前结果布局预先计算的实体字段键。只供内置 MappingPlan 跨包复用，不属于业务 API。
     */
    @InternalApi
    public String mappingKey(int index) {
        return layout.mappingKey(index);
    }

    /** 返回预先绑定的实体字段索引；只供内置 MappingPlan 使用。 */
    @InternalApi
    public int mappingIndexOf(String key) {
        return layout.mappingIndexOf(Objects.requireNonNull(key, "mapping column key must not be null"));
    }

    /** 返回当前结果布局是否含有归一化后冲突的列标签。 */
    @InternalApi
    public boolean hasAmbiguousMappingColumns() {
        return layout.hasAmbiguousMappingColumns();
    }

    /**
     * 在当前结果布局的生命周期内按映射计划身份创建一次字段绑定。
     * 只供内置 MappingPlan 跨包协作，不进入全局缓存。
     */
    @InternalApi
    public <T> T mappingBinding(Object plan, Supplier<? extends T> factory) {
        return layout.mappingBinding(plan, factory);
    }
    ...
    public DynamicRow renameColumns(UnaryOperator<String> renamer) {
        ...
        return owned(renamedLayout(safeRenamer), values);
    }

    /**
     * 为内置、不可变的列改名计划绑定一次目标布局。普通调用方继续使用 {@link #renameColumns(UnaryOperator)}，
     * 其 renamer 每次都会执行；本入口只服务 SQL 别名和结果保护等确定性内部计划。
     */
    @InternalApi
    public DynamicRow renameColumnsBound(UnaryOperator<String> renamer) {
        UnaryOperator<String> safeRenamer = Objects.requireNonNull(renamer, "column renamer must not be null");
        RowLayout renamedLayout = layout.mappingBinding(safeRenamer, () -> renamedLayout(safeRenamer));
        return owned(renamedLayout, values);
    }

    private RowLayout renamedLayout(UnaryOperator<String> renamer) {
        java.util.ArrayList<String> names = new java.util.ArrayList<>(values.length);
        for (int index = 0; index < values.length; index++) {
            names.add(renamer.apply(layout.columnName(index)));
        }
