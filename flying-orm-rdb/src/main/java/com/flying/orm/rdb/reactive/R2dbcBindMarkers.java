...
final class R2dbcBindMarkers {
    ...
    static R2dbcBindMarkers from(RdbDialect dialect) {
        return new R2dbcBindMarkers(Objects.requireNonNull(dialect, "rdb dialect must not be null").name());
    }
    ...
    /** 在批量订阅边界复验并编译一次，所有输入缓冲复用同一传输 SQL。 */
     ...
     * PostgreSQL、Oracle 和 H2 先遵守未引用名称的大小写折叠，保留未引用标识符的大小写语义。
     */
