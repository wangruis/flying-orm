...
public final class LargeObjectValueCodec {
    ...
    public static boolean isLargeObjectDataType(String dataType) {
        return isLargeObjectDataType(DatabaseType.of(dataType));
    }

    public static boolean isLargeObjectDataType(DatabaseType dataType) {
        return isBinaryDataType(dataType) || isTextDataType(dataType);
    }

    public static boolean isBinaryDataType(String dataType) {
        return isBinaryDataType(DatabaseType.of(dataType));
    }

    public static boolean isBinaryDataType(DatabaseType dataType) {
        return switch (requireType(dataType).baseName()) {
            case "BLOB", "BINARY", "VARBINARY", "TINYBLOB", "MEDIUMBLOB", "LONGBLOB",
                    "MYSQL_BINARY", "MYSQL_BLOB", "BYTEA", "RAW", "LONG RAW", "PROTECTED_BINARY" -> true;
            default -> false;
        };
    }

    public static boolean isTextDataType(String dataType) {
        return isTextDataType(DatabaseType.of(dataType));
    }

    public static boolean isTextDataType(DatabaseType dataType) {
        return switch (requireType(dataType).baseName()) {
    ...
    public static Object write(Object value, String dataType) {
        return write(value, DatabaseType.of(dataType), "");
    }
    ...
    public static Object write(Object value, DatabaseType dataType, String dialectName) {
        ...
        if (isBinaryDataType(dataType)) {
            ...
            // 包装后的载荷对 Core 不透明；借入值在此隔离，Byte[] 转换结果已由本次调用拥有。
            return new SqlTypedValue(SqlTypedValue.Kind.BLOB, value instanceof Byte[]
                    ? binaryValue : BindableValueSnapshots.immutableValue(binaryValue));
        }
        if (isTextDataType(dataType)) {
            if (value instanceof CharSequence || value instanceof char[]) {
                ...
                SqlTypedValue.Kind kind = "NCLOB".equals(dataType.baseName())
    ...
    public static Object read(Object value, String dataType) {
        return read(value, DatabaseType.of(dataType));
    }

    public static Object read(Object value, DatabaseType dataType) {
        if (value == null) {
            return null;
        }
        if (isBinaryDataType(dataType)) {
     ...
     * 执行时限由上层生命周期管理，ORM 只应用字段物化容量限制。
     */
    public static Mono<Object> readReactive(Object value,
                                            String dataType,
                                            SqlExecutionOptions options) {
        return readReactive(value, DatabaseType.of(dataType), options);
    }
    ...
    private static DatabaseType requireType(DatabaseType dataType) {
        return Objects.requireNonNull(dataType, "large object data type must not be null");
    }
