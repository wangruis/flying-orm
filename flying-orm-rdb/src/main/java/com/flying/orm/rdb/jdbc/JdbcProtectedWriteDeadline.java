...
final class JdbcProtectedWriteDeadline {
    ...
    private JdbcProtectedWriteDeadline(SqlExecutionOptions options) {
        ...
        this.timeoutNanos = saturatingNanos(options.timeout());
    }
    ...
    private long remainingNanos() {
        ...
        if (remaining <= 0L) {
            throw new SqlExecutionTimeoutException(
                    options.timeout(), new TimeoutException("jdbc protected write timed out"));
        }
        return remaining;
    }

    private static long saturatingNanos(Duration timeout) {
        if (timeout.isZero()) {
