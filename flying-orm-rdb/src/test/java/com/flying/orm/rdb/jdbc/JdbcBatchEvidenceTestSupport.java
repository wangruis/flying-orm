...
final class JdbcBatchEvidenceTestSupport {
    ...
    static final class State {
        ...
        final java.util.List<com.flying.orm.core.sql.render.SqlRequest> requests = new java.util.ArrayList<>();
        ...
        JdbcConnectionAccess access() {
            return new JdbcConnectionAccess() {
                public Connection getConnection(com.flying.orm.core.sql.render.SqlRequest request) throws java.sql.SQLException {
                    acquired.incrementAndGet();
                    requests.add(request);
                    if (acquisitionFailure instanceof java.sql.SQLException sql) throw sql;
                    if (acquisitionFailure instanceof RuntimeException runtime) throw runtime;
                    return connection();
                }
                public void releaseConnection(Connection connection, com.flying.orm.core.sql.render.SqlRequest request)
                        throws java.sql.SQLException {
                    released.incrementAndGet();
                    org.junit.jupiter.api.Assertions.assertSame(requests.getFirst(), request);
                    if (releaseFailure instanceof java.sql.SQLException sql) throw sql;
                    if (releaseFailure instanceof RuntimeException runtime) throw runtime;
                    if (releaseFailure instanceof Error error) throw error;
                }
            };
        }

        JdbcBatchWriter writer() {
            return JdbcBatchWriter.create(access(), com.flying.orm.rdb.dialect.RdbDialect.h2());
        }
        ...
        private PreparedStatement statement() {
            ...
            return proxy(PreparedStatement.class, (self, method, arguments) -> switch (method.getName()) {
                case "setObject", "setNull", "setBinaryStream", "setQueryTimeout", "cancel" -> null;
                case "close" -> {
